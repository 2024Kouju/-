...
public class ChangeDamage : MonoBehaviour
{
    ...
    void ChangeBulletDamage()
    {
        ...
        if (CoinManager.coinCount >= currentCost && AttckBullet.damage < 70f)//コインの枚数が値段より高く　弾の威力が50f以下なら中の処理を実行
        {
            // ダメージ効果音を再生
            ...
            if (AttckBullet.damage > 70f)
            {
                AttckBullet.damage = 70f; // 上限を50に設定
            }
        ...
        else if (AttckBullet.damage >= 70f)//自機(プレイヤー)の弾の威力が最大なら中の処理を実行
        {
            if (Lackofcoins != null)
            {
