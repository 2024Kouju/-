...
public class Bulletstrengthening : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        FriendBullet_Damage.damage = PlayerPrefs.GetFloat("BulletFriendDamage", 3f); // 保存された弾の威力を読み込む
    ...
    void ChangeBulletDamage()
    {
        ...
        if (CoinManager.coinCount >= currentCost && FriendBullet_Damage.damage < 5f)//コインの枚数が値段より高く　弾の威力が3f以下なら中の処理を実行
        {
            // ダメージ効果音を再生
            ...
            if (FriendBullet_Damage.damage > 5f)
            {
                FriendBullet_Damage.damage = 5f; // 上限を3に設定
            }
            if(FriendBullet_Damage.damage == 5f)
            {
                Buy.SetActive(false);
        ...
        else if (FriendBullet_Damage.damage >= 5f)//味方機体の弾の威力が最大なら中の処理を実行
        {
            if (Lackofcoins != null)
            {
